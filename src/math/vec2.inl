...
vec2<std::common_type_t<L, R>> operator*(L lhs, const vec2<R> &rhs) {
	return vec2<std::common_type_t<L, R>>(lhs * rhs.x, lhs * rhs.y);
}
...
vec2<Ret> reflect(const vec2<Incident> &incident,
		const vec2<Normal> &normal) {
...
template<soft_bodies::math::scalar T>
std::size_t hash<soft_bodies::math::vec2<T>>::operator()(soft_bodies::math::vec2<T> vec) const {
