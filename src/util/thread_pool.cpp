...
thread_pool::thread_pool(uint32_t thread_count) {
	...
	if (thread_count == 0) {
		SOFT_BODIES_LOG_WARNING(
		    "std::thread::hardware_concurrency() is not supported, falling "
		    "back to 8 execution threads."
		)
		thread_count = 8;
	}
...
void future::wait() const {
	...
	if (pool.thread_id_to_index.contains(id)) {
		// Continue some other work if it's a worker thread that requested to
		// wait
