...
std::vector<std::shared_ptr<monitor>> monitor::enumerate() {
	...
	SOFT_BODIES_ASSERT(glfw_monitors, "Failed to enumerate monitors.")
...
monitor::monitor(GLFWmonitor *glfw_monitor) : _glfw_monitor(glfw_monitor) {
	SOFT_BODIES_ASSERT(glfw_monitor, "GLFW monitor must not be NULL.")
	SOFT_BODIES_ASSERT(
	    glfw_video_mode = glfwGetVideoMode(glfw_monitor),
