...
window::window(std::string &&title, uvec2 size) {
	...
	SOFT_BODIES_ASSERT(
	    _glfw_window, ([]() {
		    ...
		    return std::string{"Failed to create GLFW window: "} + description;
	    })()
	);
	...
	glfwGetFramebufferSize(
	    _glfw_window, &glfw_frame_size.x, &glfw_frame_size.y
	);
...
void window::process() {
	...
	//	glfwSetCursorPos(_glfw_window, lastCursorPos.getX(),
	// lastCursorPos.getY()); } else lastCursorPos = newPos;
...
void window::_construct_swapchain() {
	SOFT_BODIES_ASSERT(
	    !surface_constructed,
	    "Window already has a swapchain. It must be destroyed before creating "
	    "a new one."
	);
...
void window::mouse_button_callback(
    GLFWwindow *glfw_window, int32_t button, int32_t action, int32_t mods
) {}
...
void window::pos_callback(GLFWwindow *glfw_window, int32_t x, int32_t y) {
	window *window =
	    static_cast<os::window *>(glfwGetWindowUserPointer(glfw_window));

	// We can't rely on the tracking variables as callbacks may fire in the
	// wrong order
	bool maximized =
	    (glfwGetWindowAttrib(glfw_window, GLFW_MAXIMIZED) == GLFW_TRUE);
	bool minimized =
	    (glfwGetWindowAttrib(glfw_window, GLFW_ICONIFIED) == GLFW_TRUE);
...
void window::size_callback(
    GLFWwindow *glfw_window, int32_t width, int32_t height
) {
	window *window =
	    static_cast<os::window *>(glfwGetWindowUserPointer(glfw_window));
...
void window::iconify_callback(GLFWwindow *glfw_window, int32_t iconified) {
	static_cast<window *>(glfwGetWindowUserPointer(glfw_window))->minimized =
	    iconified;
}

void window::maximize_callback(GLFWwindow *glfw_window, int32_t maximized) {
	static_cast<window *>(glfwGetWindowUserPointer(glfw_window))->maximized =
	    maximized;
}
